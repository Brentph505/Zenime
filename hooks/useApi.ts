...
import { getInfoCacheConfig, normalizeAnimeStatus } from '../lib/caching/cacheConfig';
...
export async function fetchAnimeInfo(
  ...
  try {
    ...
    if (info) {
      // Apply status-aware cache immediately after success
      await applyStatusAwareCaching(animeId, info);
      return info;
    }
  } catch (error) {
    console.log(`⚠️ Error from ${finalProvider}...`, error);
  ...
  if (isHentai) {
    ...
    if (canTryHentaimama) {
      try {
        ...
        if (hInfo) {
          await applyStatusAwareCaching(animeId, hInfo);
          return hInfo;
        }
      } catch (error) {
  ...
  if (canTryReanime) {
    try {
      ...
      if (reanimeInfo) {
        await applyStatusAwareCaching(animeId, reanimeInfo);
        return reanimeInfo;
      }
    } catch (error) {
      console.log(`⚠️ Error from reanime...`, error);
  ...
  if (canTryKickassanime) {
    try {
      ...
      if (kickassInfo) {
        await applyStatusAwareCaching(animeId, kickassInfo);
        return kickassInfo;
      }
    } catch (error) {
      console.log(`⚠️ Error from kickassanime...`, error);
  ...
  if (canTryAnimePahe) {
    try {
      ...
      if (paheInfo) {
        await applyStatusAwareCaching(animeId, paheInfo);
        return paheInfo;
      }
    } catch (error) {
      console.log(`⚠️ Error from animepahe...`, error);
...
/**
 * Apply status-aware caching to anime info.
 * This ensures that "Not yet released" anime are not permanently cached,
 * so they update when they start airing.
 */
async function applyStatusAwareCaching(animeId: string, info: any): Promise<void> {
  try {
    ...
    // Re-cache with status-aware config instead of permanent
    await cacheManager.set('Info', animeId, info, { strategy: config.strategy, ttl: config.ttl, backend: config.backend, updatePolicy: config.updatePolicy, priority: config.priority });
    
    if (status === 'NOT_YET_RELEASED') {
      console.log(`📌 [Cache] Anime ${animeId} is NOT_YET_RELEASED — using short TTL (1h) for quick refresh`);
    }
  } catch (err) {
    console.warn(`⚠️ [Cache] Failed to apply status-aware caching for ${animeId}:`, err);
  }
}
